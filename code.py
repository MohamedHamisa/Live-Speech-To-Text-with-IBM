import argparse # It's what you use to get command line arguments into your program
import base64 #a binary to ASCII encoding scheme
import configparser
import json
import threading #it means that it will excute several things happen at the same time
import time

import pyaudio
import websocket # open a two-way interactive communication session between the user's browser and a server
from websocket._abnf import ABNF # augmented Backus–Naur form is a model in cs used to build sentences based on his grammer

CHUNK = 1024
FORMAT = pyaudio.paInt16 #16-bit binary string. 15 bits for the number, and one for the sign
...
def on_open(ws):
    ...
    data = {
        ...
        "word_confidence": True, # directs the service to report a confidence measure 
        "timestamps": True,
        "max_alternatives": 3
    }
...
def get_url():
    ...
    return ("wss://{}/speech-to-text/api/v1/recognize"
           "?model=en-AU_BroadbandModel").format(host) # if you want to leverage another model you can do it from ibm cloud
